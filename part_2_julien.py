...
WINDOW_SEC = 1.0     # length of each analysis window (seconds)
HOP_SEC = 0.25        # step between consecutive windows (seconds)
MIN_CONF = 0.35      # minimum confidence to accept a prediction (not noise)
...
def get_models():
    return {
        ...
        n_estimators=100,       # more trees = more stable probabilities
...
def _windows(y, sr, window_sec=WINDOW_SEC, hop_sec=HOP_SEC):
    ...
    # ← Just remove the partial window block entirely, or add a minimum length guard:
    if start < n_samples:
        remaining = n_samples - start
        if remaining >= win_samples * 0.75:  # only keep if >75% full
            chunk = np.pad(y[start:], (0, win_samples - remaining))
            yield start / sr, n_samples / sr, chunk
...
def _merge_detections(detections: list, min_gap_sec: float = MIN_CALL_SEC) -> list:
    ...
    segments = []
    cur = dict(detections[0])  # mutable copy
    ...
    # Compute summary stats and tidy up
    results = []
    for seg in segments:
...
def detect_long_audio(
    ...
    raw_detections = []   # one entry per accepted window
    ...
    if verbose:
        print(f"  Fenêtres retenues (hors bruit) : {len(raw_detections)}")

    merged = _merge_detections(raw_detections, min_gap_sec=min_call_sec)
    ...
    print(f"t={t_start:.1f}s → {dict(zip(le.classes_, proba.round(3)))}")
    return df
